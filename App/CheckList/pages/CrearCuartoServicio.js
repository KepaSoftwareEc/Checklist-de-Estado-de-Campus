...
const CrearCuartoServicio = () => {
  ...
  const handleSave = async () => {
    ...
    const newRoom = {
      ...
      roomNumber,
      image,
      electricItems: electricItems.map(item => ({
        id: item.id,
      ...
      infrastructureItems: infrastructureItems.map(item => ({
        id: item.id,
        description: item.description,
        quantity: item.quantity
      })),
      securityItems: securityItems.map(item => ({
        id: item.id,
        description: item.description,
        quantity: item.quantity
      })),
    ...
    console.log('Datos enviados:', JSON.stringify(newRoom, null, 2)); // Mensaje de depuración

    try {
      const response = await fetch('https://checklistutpl.duckdns.org/api/cuarto-servicio/crear', {
        method: 'POST',
        headers: {
      ...
      console.log('Cuarto de servicio creado:', data);
  ...
  const loadBuildings = async () => {
    try {
      ...
      const data = await response.json();
      console.log('Edificios obtenidos:', data); // Mensaje de depuración
      if (data.length === 0) {
        alert('No se encontraron edificios');
      }
      const parsedBuildings = data.map(edificio => ({
        key: edificio.id,
        value: `Edificio ${edificio.buildingNumber}`,
        name: edificio.buildingName,
        number: edificio.buildingNumber
      }));
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Agregar Cuarto de Servicio</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.container}>
          ...
          <SelectList
            setSelected={(key) => {
              const selectedBuilding = buildings.find(b => b.key === key);
              setSelectedBuilding(selectedBuilding);
              setBuilding(selectedBuilding.name);
              setBuildingNumber(selectedBuilding.number);
            }}
            data={buildings}
            placeholder="Seleccionar edificio"
            boxStyles={styles.selectBox} // Estilo del cuadro
            dropdownStyles={styles.dropdown} // Estilo del menú desplegable
            dropdownIcon={<Ionicons name="chevron-down" size={20} color="#FFD700" />} // Ícono amarillo
          />

          <Text style={styles.label}>Número de cuarto:</Text>
          <TextInput
            style={styles.input}
            value={roomNumber}
            onChangeText={setRoomNumber}
            placeholder="Ingrese número de cuarto"
          />
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  selectBox: {
    backgroundColor: '#F4F4F8', // Fondo gris claro
    borderWidth: 1,
    borderColor: '#ddd',
    borderRadius: 8,
    padding: 10,
  ...
  dropdown: {
    backgroundColor: '#F4F4F8', // Fondo gris claro para el menú desplegable
    borderWidth: 1,
    borderColor: '#ddd',
    borderRadius: 8,
    marginTop: 5,
...
export default CrearCuartoServicio;