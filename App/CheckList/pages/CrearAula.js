...
export default function Component() {
  ...
  const [floor, setFloor] = useState(''); // Nuevo estado para el piso
  ...
  const sendImageToAPI = async (imageUrl) => {
    try {
      const response = await fetch('http://34.36.202.38/', {
        method: 'POST',
        headers: {
  ...
  const addItem = (type) => {
    const newItem = { id: Date.now().toString(), description: '', quantity: '' };
    switch (type) {
      case 'electric':
  ...
  const handleSave = async () => {
    const newAula = {
      ...
      building,
      classroomNumber,
      floor, // Incluir el piso en el objeto
      image,
      electricItems,
      waterItems,
      furnitureItems,
    ...
    try {
      const response = await fetch('https://checklistutpl.duckdns.org/api/aulas/crear', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Agregar aula</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <SelectList
          ...
          dropdownIcon={<Ionicons name="chevron-down" size={20} color="#F2B705" />}
        />

        <Text style={styles.label}>   Número de aula:</Text>
        <TextInput
          style={styles.input}
        ...
        {/* Nueva sección para el piso */}
        <Text style={styles.label}>   Piso:</Text>
        <TextInput
          style={styles.input}
          value={floor}
          onChangeText={setFloor}
          placeholder="Ingrese el piso (ej: 1, 2, 3...)"
...
const styles = StyleSheet.create({
  ...
  selectBox: {
    backgroundColor: '#F4F4F8', // Fondo del cuadro
    borderWidth: 1,
    borderColor: '#ddd',
    borderRadius: 8,
    padding: 10,
  ...
  dropdown: {
    backgroundColor: '#F4F4F8', // Fondo del menú desplegable
    borderWidth: 1,
    borderColor: '#ddd',
    borderRadius: 8,
    marginTop: 5,
